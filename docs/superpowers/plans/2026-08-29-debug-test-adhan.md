...
5. **Post-review (user decision):** `scheduleTestAdhan` now respects the master "Notifications enabled" toggle — reads `dataStore.getSettings()` and skips scheduling (logs a warning) when disabled (`104c334`), and cancels any previously scheduled test alarm in the disabled branch so no stale alarm fires (`be4233b`). 2 new scheduler tests added (22 total in file).