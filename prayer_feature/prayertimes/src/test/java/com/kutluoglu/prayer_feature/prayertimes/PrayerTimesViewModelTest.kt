...
class PrayerTimesViewModelTest {
    ...
    private lateinit var dailyLoader: DailyPrayerTimesLoader
    ...
    fun `month load requests daily times without persisting per-day cache`() = runTest {
        ...
        coVerify(atLeast = currentMonthDays()) {
            dailyLoader.load(any(), any(), any(), any(), any(), JuristicMethod.STANDARD, false)
        }
    }
    ...
    fun `monthly save runs in background after success is emitted`() = runTest {
        ...
        coEvery {
            saveMonthlyPrayerTimesUseCase.invoke(any(), any(), any(), any(), any(), any(), any())
    ...
    fun `cold load streams today first then completes full sorted month`() = runTest {
        ...
        coEvery {
            dailyLoader.load(any(), any(), any(), any(), any(), any(), any())
        } coAnswers {
            val date = firstArg<LocalDateTime>()
            gates.getOrPut(date.date.dayOfMonth) { CompletableDeferred() }.await()
            success(DailyPrayerTimes(mockPrayerList, null, null, 0L, 0L, false))
        }
    ...
    fun `navigation during an in-flight load is not dropped`() = runTest {
        ...
        var callCount = 0
        coEvery { dailyLoader.load(any(), any(), any(), any(), any(), any(), any()) } coAnswers {
            callCount++
    ...
    fun `location switch during in-flight load does not emit stale pairing`() = runTest {
        ...
        coEvery { dailyLoader.load(any(), any(), any(), any(), any(), any(), any()) } coAnswers {
            ...
            if (callCount == 1) gate.await()
            if (latitude == locA.latitude) {
                success(DailyPrayerTimes(mockPrayerList, null, null, 0L, 0L, false))
            } else {
                success(DailyPrayerTimes(prayerListB, null, null, 0L, 0L, false))
            }
        }
        ...
        viewModel.uiState.test {
            ...
            assertThat(success.monthlyPrayers.first().prayers.first().name).isEqualTo("ImsakB")
            cancelAndIgnoreRemainingEvents()
        }
    }
    ...
    fun `persisted month prayer names are re-localized on load`() = runTest {
        ...
        viewModel.uiState.test {
            val state = awaitItem()
            val success = state as PrayerTimesUiState.Success
            assertThat(success.monthlyPrayers.first().prayers.first().name).isEqualTo("ImsakL")
            cancelAndIgnoreRemainingEvents()
        }
    }
