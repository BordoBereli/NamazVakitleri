...
class AdhanResIdResolverTest {
    ...
    fun `unknown style falls back to default`() {
        assertEquals(R.raw.adhan_dhuhr, resolver.resolve("Dhuhr", "bogus_style"))
    }

    @Test
    fun `unknown prayer falls back to dhuhr`() {
        assertEquals(R.raw.adhan_dhuhr, resolver.resolve("Unknown", AdhanStyle.DEFAULT.id))
    }
}
